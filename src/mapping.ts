...
export class MappingStore {
  private sessions = new Map<string, SessionMapEntry>()
  /** 日志打码专用（无会话语义，不还原不落盘）。 */
  readonly logMap: MaskMap = createMaskMap()
  private stats: RedactStats = { categories: {} }
  ...
  mergePersistedStat(code: string, count: number, lastAt: number | undefined): void {
    ...
    if (lastAt !== undefined && (stat.lastAt === undefined || lastAt > stat.lastAt)) stat.lastAt = lastAt
  ...
  prune(now: number, ttlMs = SESSION_TTL_MS, maxSessions = MAX_SESSIONS): number {
    let removed = 0
  ...
  toPersistable(): PersistedMaps {
    const sessions: Record<string, { lastActive: number; reverse: Record<string, string> }> = {}
    for (const [id, entry] of this.sessions) {
  ...
  loadPersistable(data: PersistedMaps, now: number): void {
    ...
    for (const [id, saved] of Object.entries(data.sessions ?? {})) {
      ...
      let maxByCode = new Map<string, number>()
      ...
      const entry: SessionMapEntry = { map, lastActive: typeof saved.lastActive === 'number' ? saved.lastActive : now }
