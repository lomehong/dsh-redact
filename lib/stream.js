 ...
 */
import { extractAliasEntries, maskText, restoreAll, } from "./rules.js";
/* ─────────────── 出站：消息遍历脱敏 ─────────────── */
...
export class PlaceholderRestorer {
    ...
    constructor(reverse, options) {
        ...
        this.aliasHold = this.aliasEntries.length > 0
            ...
            : 0;
        if (options?.onRestore !== undefined)
...
export function holdbackIndex(pending) {
    ...
    for (let i = windowStart; i < pending.length; i++) {
        ...
        if (PLACEHOLDER_PREFIX_RE.test(tail) || PLACEHOLDER_ONE_BRACKET_RE.test(tail))
            return i;
    }
    // 孤立的结尾 '[' 也可能是跨 chunk 的 '[['
    if (pending.endsWith('['))
...
export function restoreBlock(block, reverse, aliasEntries) {
    if (block === null || typeof block !== 'object')
        return block;
    switch (block.type) {
        case 'text':
        case 'reasoning':
            return { ...block, text: restoreAll(String(block.text ?? ''), reverse, aliasEntries) };
        case 'tool-call':
            return { ...block, arguments: restoreAll(String(block.arguments ?? ''), reverse, aliasEntries) };
        case 'tool-result':
            return { ...block, content: block.content.map((b) => restoreBlock(b, reverse, aliasEntries)) };
        default:
            return block;
    }
}
/** 包装 chunk 流做入站还原；任何异常透传原始 chunk（宁可不还原，不可断流）。 */
export async function* restoreChunks(chunks, restorer, reverse, aliasEntries) {
        ...
        try {
            switch (chunk?.type) {
                ...
                case 'block-end': {
                    ...
                    yield { ...chunk, block: restoreBlock(chunk.block, reverse, aliasEntries) };
                    break;
                }
                case 'finish': {
