...
export function stateFilePath(homeDir) {
...
export async function loadState(homeDir) {
    try {
        const raw = JSON.parse(await readFile(stateFilePath(homeDir), 'utf8'));
        if (raw === null || typeof raw !== 'object' || raw.version !== 1)
            return undefined;
        if (raw.maps === null || typeof raw.maps !== 'object')
            return undefined;
        if (raw.stats === null || typeof raw.stats !== 'object')
            return undefined;
        return raw;
    }
    catch {
...
export async function saveState(homeDir, state) {
